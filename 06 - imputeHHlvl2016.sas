...
/* using IMPUTED_FINAL_2016 datasets with SIZE_NEW, SIZE_IMPUTED_MEAN, SIZE_IMPUTED_MEDIAN, SIZE_IMPUTED_MODE */
/* load back into SAS our permanent copy */
DATA imputed_final_2016;
SET cfoy.imputed_final_2016;
RUN;

/* level of processing */
/* LOP - expenditure, and weights (size new, mean, median and mode) */
DATA imputed_final_2016;
SET imputed_final_2016;
...
/* permanent file */
DATA cfoy.imputed_final_2016;
SET imputed_final_2016;
RUN;

/* Total expenditure and total purchased weight for each HH */
PROC SQL;
CREATE TABLE totalsperHH2016 AS
...
FROM imputed_final_2016
GROUP BY HH
ORDER BY HH;
QUIT;

/* create "year" column */
DATA totalsperHH2016;
SET totalsperHH2016;
Year = 2016;
RUN;
...
/* permanent copy */
DATA cfoy.totalsperHH2016;
SET totalsperHH2016;
RUN;

DATA totalsperHH2016;
SET cfoy.totalsperHH2016;
RUN;
/****************************************************************/
/* END*/
...
RUN;
DATA totalsperHH2014;
SET cfoy.totalsperHH2014;
RUN;
DATA totalsperHH2015;
SET cfoy.totalsperHH2015;
RUN;
DATA totalsperHH2016;
SET cfoy.totalsperHH2016;
RUN;

/* sort all reduced datasets by "YEAR" then "HH" */
%sortit(totalsperHH2012, Year HH);
%sortit(totalsperHH2013, Year HH);
%sortit(totalsperHH2014, Year HH);
%sortit(totalsperHH2015, Year HH);
%sortit(totalsperHH2016, Year HH);
QUIT;

/* merge all reduced datasets into ONE BIG DATASET = one row per HH per year */
DATA reduced_final;
SET totalsperHH2012 totalsperHH2013 totalsperHH2014 totalsperHH2015 totalsperHH2016;
RUN;

DATA reduced_final;
RETAIN Year HH Weight Lifestage Income_Group Income_new HH_Size HH_size_new Age_Group Age_new Sex Sex_cat;
SET reduced_final;
RUN;
