...
DATA homescan_2012;
SET cfoy.homescan_2012_final;
RUN;

/* 2012 total household expenditure sum(QTY * PRICE) */
PROC SQL;
TITLE 'Homescan dataset (2012): Total household expenditure';
...
PROC FREQ DATA = homescan_2012 nlevels;
TABLES UNIT;
TITLE 'Homescan dataset (2012): UNIT from Nielsen panel';
RUN;
/*  (2 levels g or ml + missing level) different levels for size unit */
PROC FREQ DATA = homescan_2012 nlevels;
TABLES Packet_Unit;
TITLE 'Homescan dataset (2012): Packet Unit from Supermarket database';
RUN;
...
/* permanent file */
DATA cfoy.varsaddedhomescan_2012;
SET varsaddedhomescan_2012;
RUN;

/* checking Item_unit and Item_size (96,542 obs were missing a unit and weight) */
	/* 96,542/1,624,714 = approx 5.94% obs missing in item size and unit */
PROC MEANS DATA = varsaddedhomescan_2012 NMISS N;
VAR Item_size;
RUN;
PROC FREQ DATA = varsaddedhomescan_2012 nlevels;
TABLES Item_unit;
TITLE 'Homescan dataset (2012): Item Units';
RUN;
...
DATA missing_size_2012;
SET varsaddedhomescan_2012 (KEEP = HH WEIGHT Barcode BARCODE_DESC SIZE UNIT link Packet_Size Packet_Unit
...
RUN;
PROC SORT DATA=missing_size_2012 OUT=cfoy.missing_size_2012 (DROP = Packet_Size Packet_Unit Item_size Item_unit);
BY UNIT;
RUN;
/* export as excel file */
PROC EXPORT DATA = cfoy.missing_size_2012
...
/****************************************************************************/
/* combine "extract2012.csv" into master dataset "varsaddedhomescan_2012" */
/****************************************************************************/
/* import "extract2012.csv" file into SAS */
...
PROC IMPORT DATAFILE = "F:\x Honours Project\02 - Statistical analysis plan (SAP)\003 - Missing sizes\extract2012.csv" OUT = extracted_2012
DBMS = csv;
GETNAMES = YES;
GUESSINGROWS = 96543;
RUN;
...
DATA extracted_2012;
SET extracted_2012 (KEEP = HH Weight BARCODE BARCODE_DESC SIZE_new UNIT_new);
...
/* modify the master dataset "varsaddedhomescan_2012" with transaction dataset "extracted_2012" */
/* FIRST: create "uniquebar2012" table that contains all the possible unique barcodes, barcodes_desc, size and unit */
PROC SQL;
CREATE TABLE uniquebar2012 AS
...
CREATE TABLE onemerge_2012 AS
SELECT A.*, B.SIZE_new, B.UNIT_new
FROM varsaddedhomescan_2012 A LEFT JOIN uniquebar2012 B
ON A.Barcode = B.Barcode
ORDER BY HH;
QUIT;

PROC FREQ DATA = onemerge_2012;
TABLE SIZE_New;
RUN;
...
DATA final_2012;
SET final_2012 (DROP = Item_size Item_unit);
...
/* (YES!) CHECK items have been read properly and the total items still missig size is 70,894 */
PROC FREQ DATA=final_2012;
...
/* EXPORT FINAL RAW DATASET FOR 2012 as permanent SAS table file */
/* final raw dataset as permanent file */
DATA cfoy.final_2012;
SET final_2012;
RUN;
