...
/* using IMPUTED_FINAL_2012 datasets with SIZE_NEW, SIZE_IMPUTED_MEAN, SIZE_IMPUTED_MEDIAN, SIZE_IMPUTED_MODE */
/* load back into SAS our permanent copy */
DATA imputed_final_2012;
SET cfoy.imputed_final_2012;
RUN;

PROC EXPORT DATA=imputed_final_2012
OUTFILE = "F:\x Honours Project\02 - Statistical analysis plan (SAP)\007 - IMPUTE - expenditure and weight for each HH per year/imputed_final_2012.csv" REPLACE
DBMS = csv;
RUN;
...
PROC SQL;
CREATE TABLE totalsperHH2012 AS
...
FROM imputed_final_2012
GROUP BY HH
ORDER BY HH;
QUIT;

/* create "year" column */
DATA totalsperHH2012;
SET totalsperHH2012;
Year = 2012;
RUN;
...
/* permanent copy */
DATA cfoy.totalsperHH2012;
SET totalsperHH2012;
RUN;

DATA totalsperHH2012;
SET cfoy.totalsperHH2012;
RUN;
/****************************************************************/
/* END*/
...
/**** CHECKS: compare to R produced table ****/
PROC IMPORT OUT = totalsbyR
	DATAFILE = "E:\x Honours Project\02 - Statistical analysis plan (SAP)\007 - IMPUTE - expenditure and weight for each HH per year\totalsbyR.xlsx"
	DBMS = xlsx;
	GETNAMES = YES;
RUN;
...
DATA totalsbyR;
SET totalsbyR;
FORMAT totalexp BEST12.;
FORMAT totalwgtSIZEnew BEST12.;
FORMAT totalwgtSIZEmean BEST12.;
FORMAT totalwgtSIZEmedian BEST12.;
FORMAT totalwgtSIZEmode BEST12.;
RUN;

DATA totalsbyR;
SET totalsbyR;
FORMAT totalexp 8.;
FORMAT totalwgtSIZEnew 8.;
FORMAT totalwgtSIZEmean 8.;
FORMAT totalwgtSIZEmedian 8.;
FORMAT totalwgtSIZEmode 8.;
RUN;

PROC COMPARE BASE = totalsperHH2012 COMPARE=totalsbyR;
	VAR HH_Total_Expenditure;
	WITH totalexp;
RUN;

PROC COMPARE BASE = totalsperHH2012 COMPARE=totalsbyR;
	VAR HH_SIZE_new_weight;
	WITH totalwgtSIZEnew;
RUN;

PROC COMPARE BASE = totalsperHH2012 COMPARE=totalsbyR;
	VAR HH_SIZE_mean_weight;
	WITH totalwgtSIZEmean;
RUN;
/***************************/
