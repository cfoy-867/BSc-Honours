...
/* Ntotals */
/* N_unqi = total # of unique products from final_2013 (regardless with MISSING or NOT missing size) */
...
/* n_size = # of unique products from final_2013 (WITH SIZE i.e. SIZE_new is not missing) */
...
PROC MEANS DATA = allsubset2013  MAXDEC = 2 n mean median mode;
...
/* combine */
PROC SQL;
CREATE TABLE testing2013 AS
SELECT L.*, R.*
FROM testcombine L
LEFT JOIN missstats_2013 R
...
/* uniqpostmissing_2013 = final table with info for possible imputation */
PROC SQL;
CREATE TABLE uniqpostmissing_2013 AS
SELECT y.*,
	(nmiss/N_unqi*100) AS nmiss_prop format 8.2,
	(nmiss_exp/N_exp*100) AS nmiss_exp_prop format 8.2
FROM combine AS y;
QUIT;
DATA alluniqpostmissing_2013;
...
SET uniqpostmissing_2013;
RUN;

PROC SORT DATA = alluniqpostmissing_2013;
BY Food_Group_Code_1 Food_Group_Code_3;
RUN;

/* export as xlsx - permanent copy */
DATA cfoy.alluniqpostmissing_2013;
SET alluniqpostmissing_2013;
RUN;

/* export as excel file */
PROC EXPORT DATA = alluniqpostmissing_2013
...
WHERE nmiss ne .;
RUN;
PROC EXPORT DATA = unqimiss_2013
...
PROC SQL;
CREATE TABLE rulesapplied_2013 AS
SELECT * FROM alluniqpostmissing_2013
WHERE nmiss_prop <= 25
...
/* export as xlsx - permanent copy */
DATA cfoy.rulesapplied_2013;
SET rulesapplied_2013;
RUN;

/* export as excel file */
PROC EXPORT DATA = rulesapplied_2013
...
/* CREATE table of remaining items with missing sizes = 82,535 */
DATA postmissing_2013;
SET final_2013;
...
PROC SQL;
CREATE TABLE unqi2013 AS
SELECT DISTINCT Food_Group_Name_1, Food_Group_Code_1, Food_Group_Name_2, Food_Group_Code_2, Food_Group_Name_3, Food_Group_Code_3
FROM postmissing_2013;
QUIT;
...
FROM unqi2013 as x, final_2013 as y
...
PROC SQL;
CREATE TABLE uni_subset2013 AS
...
/* calculate mean median and mode using UNIQUE PRODUCTS = uni_subset2013 */
/* CREATE MACRO to add number of observations from master that have sizes that were used to calculate mean, median and mode */
%MACRO select(foodgroup=);
%IF %length(&foodgroup) = 6 %THEN %DO;
PROC MEANS DATA = uni_subset2013  MAXDEC = 2 n mean median mode;
TITLE "Statistics for Food Group 3";
VAR SIZE_new;
WHERE Food_Group_Code_3 = "&foodgroup";
OUTPUT out = new(drop = _type_ _freq_) n=n mean=mean median=median mode=mode;
...
/* execute MACRO for all 25 FG levels that have missing sizes*/
%select(foodgroup = 010101)
%select(foodgroup = 010204)
%select(foodgroup = 010206)
%select(foodgroup = 010302)
...
/* table of statistics of missing items */
DATA missstats_2013;
SET miss010101 miss010204 miss010206 miss010302 miss010305 miss010304 miss0104 miss0405 miss040201 miss050607 miss07 miss080106 miss080107 miss090205 miss090506 miss100204 miss100218 miss100217 miss100216 miss110101 miss110201 miss110501 miss1501 miss1506 miss17;
RUN;
...
FROM unqi2013 as x, subset2013 as y
WHERE y.Food_Group_Name_1=x.Food_Group_Name_1 
	AND y.Food_Group_Name_2=x.Food_Group_Name_2
	AND y.Food_Group_Name_3=x.Food_Group_Name_3
...
/* export as xlsx - permanent copy */
DATA cfoy.unqimiss_2013;
SET uniqpostmissing_2013;
RUN;

/* export as excel file */
PROC EXPORT DATA = uniqpostmissing_2013
...
%MACRO select(foodgroup=);
%IF %length(&foodgroup) = 6 %THEN %DO;
PROC MEANS DATA = final_2013  MAXDEC = 2 nmiss n mean median mode;
TITLE "Statistics for Food Group 3";
VAR SIZE_new;
WHERE Food_Group_Code_3 = "&foodgroup";
...
DATA cfoy.itemsmiss_2013;
SET itemsmiss_2013;
RUN;

/* export as excel file */
PROC EXPORT DATA = itemsmiss_2013
...
ODS HTML BODY = "postmissing2013-body.html";

PROC FREQ DATA = final_2013;
	TABLES PROCESSING_CODE;
	WHERE SIZE_new in (., 0) ;
	TITLE 'Homescan dataset (2013): Level of processing frequency of missing Item sizes';
RUN;
...
PROC FREQ DATA = final_2013;
	...
	WHERE SIZE_new in (., 0) ;
	TITLE 'Homescan dataset (2013): Food Group 1 frequency of missing Item sizes';
RUN;
...
PROC FREQ DATA = final_2013;
	...
	WHERE SIZE_new in (., 0) ;
	TITLE 'Homescan dataset (2013): Food Group 2 frequency of missing Item sizes';
RUN;
/* FG3 missing sizes */
PROC FREQ DATA = final_2013;
	TABLES Food_Group_Name_3;
	WHERE SIZE_new in (., 0) ;
	TITLE 'Homescan dataset (2013): Food Group 3 frequency of missing Item sizes';
RUN;

/* 2013 total household weight purchased (kg = g / 1000) = 1,259,857 kg*/
PROC SQL;
TITLE 'Homescan dataset (2013): Total household weight purchased (kg)';
SELECT SUM(QTY*SIZE_new/1000) as WeightPurchased
FROM final_2013;
QUIT;

/* 2013 total household expenditure $8,741,583*/
PROC SQL;
TITLE 'Homescan dataset (2013): Total household expenditure (all items $)';
SELECT SUM(QTY*PRICE) as TotalExpenditure
FROM final_2013;
QUIT;

/* calculate total expenditure and proportion for items with missing sizes */
/* $ 297,272.10 / $8,741,583 total = 3.4% */
PROC SQL;
TITLE 'Homescan dataset (2013): Total household expenditure from items with missing sizes (82,535 items)';
SELECT SUM(QTY*PRICE) as MissingExpenditure,
SUM(QTY*PRICE)/8741583*100 as ExpenditurePercentage
FROM postmissing_2013;
QUIT;
