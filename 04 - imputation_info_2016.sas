...
/* Ntotals */
/* N_unqi = total # of unique products from final_2016 (regardless with MISSING or NOT missing size) */
...
/* n_size = # of unique products from final_2016 (WITH SIZE i.e. SIZE_new is not missing) */
...
/* calculate mean median and mode using UNIQUE PRODUCTS = allsubset2016 */
...
PROC MEANS DATA = allsubset2016  MAXDEC = 2 n mean median mode;
...
/* combine */
PROC SQL;
CREATE TABLE testing2016 AS
SELECT L.*, R.*
FROM testcombine L
LEFT JOIN missstats_2016 R
...
/* uniqpostmissing_2016 = final table with info for possible imputation */
PROC SQL;
CREATE TABLE uniqpostmissing_2016 AS
SELECT y.*,
	(nmiss/N_unqi*100) AS nmiss_prop format 8.2,
	(nmiss_exp/N_exp*100) AS nmiss_exp_prop format 8.2
FROM combine AS y;
QUIT;
DATA alluniqpostmissing_2016;
...
SET uniqpostmissing_2016;
RUN;

PROC SORT DATA = alluniqpostmissing_2016;
BY Food_Group_Code_1 Food_Group_Code_3;
RUN;

/* export as xlsx - permanent copy */
DATA cfoy.alluniqpostmissing_2016;
SET alluniqpostmissing_2016;
RUN;

/* export as excel file */
PROC EXPORT DATA = alluniqpostmissing_2016
...
WHERE nmiss ne .;
RUN;
PROC EXPORT DATA = unqimiss_2016
...
PROC SQL;
CREATE TABLE rulesapplied_2016 AS
SELECT * FROM alluniqpostmissing_2016
WHERE nmiss_prop <= 25
...
/* export as xlsx - permanent copy */
DATA cfoy.rulesapplied_2016;
SET rulesapplied_2016;
RUN;

/* export as excel file */
PROC EXPORT DATA = rulesapplied_2016
...
/* CREATE table of remaining items with missing sizes = 90,852 */
DATA postmissing_2016;
SET final_2016;
WHERE SIZE_new IS MISSING;
RUN;

/* unique items by FG levels 1, 2 and 3 that have MISSING SIZE from postmissing_2016 */
PROC SQL;
CREATE TABLE unqi2016 AS
SELECT DISTINCT Food_Group_Name_1, Food_Group_Code_1, Food_Group_Name_2, Food_Group_Code_2, Food_Group_Name_3, Food_Group_Code_3
FROM postmissing_2016;
QUIT;
...
FROM unqi2016 as x, final_2016 as y
...
PROC SQL;
CREATE TABLE uni_subset2016 AS
...
/* calculate mean median and mode using UNIQUE PRODUCTS = uni_subset2016 */
/* CREATE MACRO to add number of observations from master that have sizes that were used to calculate mean, median and mode */
%MACRO select(foodgroup=);
%IF %length(&foodgroup) = 6 %THEN %DO;
PROC MEANS DATA = uni_subset2016  MAXDEC = 2 n mean median mode;
TITLE "Statistics for Food Group 3";
VAR SIZE_new;
WHERE Food_Group_Code_3 = "&foodgroup";
OUTPUT out = new(drop = _type_ _freq_) n=n mean=mean median=median mode=mode;
...
/* execute MACRO for all 38 FG levels that have missing sizes*/
%select(foodgroup = 16)
%select(foodgroup = 010101)
%select(foodgroup = 010103)
%select(foodgroup = 010204)
...
/* table of statistics of missing items */
DATA missstats_2016;
length Food_Group_Code $6;
SET miss16 miss010101 miss010103 miss010204 miss010206 miss010302 miss010305 miss010304 miss0104 miss030103 miss030102
miss0405 miss040201 miss040303 miss040304 miss040402 miss050301 miss050403 miss050607 miss07 miss080101 miss080106
miss080107 miss090205 miss090401 miss090506 miss090503 miss100218 miss100217 miss100216 miss110101 miss110201 
miss110501 miss130104 miss1501 miss1502 miss1506 miss17;
RUN;
...
FROM unqi2016 as x, subset2016 as y
WHERE y.Food_Group_Name_1=x.Food_Group_Name_1 
	AND y.Food_Group_Name_2=x.Food_Group_Name_2
	AND y.Food_Group_Name_3=x.Food_Group_Name_3
...
/* export as xlsx - permanent copy */
DATA cfoy.unqimiss_2016;
SET uniqpostmissing_2016;
RUN;

/* export as excel file */
PROC EXPORT DATA = uniqpostmissing_2016
...
%MACRO select(foodgroup=);
%IF %length(&foodgroup) = 6 %THEN %DO;
PROC MEANS DATA = final_2016  MAXDEC = 2 nmiss n mean median mode;
TITLE "Statistics for Food Group 3";
VAR SIZE_new;
WHERE Food_Group_Code_3 = "&foodgroup";
...
/* table of statistics of missing items */
DATA missstats_2016;
SET miss16 miss010101 miss010103 miss010204 miss010206 miss010302 miss010305 miss010304 miss0104 miss030103 miss030102 miss0405 miss040201 miss040303 miss040304 miss040402 miss050301 miss050403 miss050607 miss07 miss080101 miss080106 miss080107 miss090205 miss090401 miss090506 miss090503 miss100218 miss100217 miss100216 miss110101 miss110201 miss110501 miss130104 miss1501 miss1502 miss1506 miss17;
RUN;
...
/* export as excel file */
PROC EXPORT DATA = itemsmiss_2016
...
ODS HTML BODY = "postmissing2016-body.html";

/**** which LOP and FG1 level has missing item_size? Frequency and percentages ***/
PROC FREQ DATA = final_2016;
	TABLES PROCESSING_CODE;
	WHERE SIZE_new in (., 0) ;
	TITLE 'Homescan dataset (2016): Level of processing frequency of missing Item sizes';
RUN;

PROC FREQ DATA = final_2016;
	TABLES Food_Group_Code_1;
	WHERE SIZE_new in (., 0) ;
	TITLE 'Homescan dataset (2016): Food Group 1 frequency of missing Item sizes';
RUN;
...
PROC FREQ DATA = final_2016;
	...
	WHERE SIZE_new in (., 0) ;
	TITLE 'Homescan dataset (2016): Food Group 2 frequency of missing Item sizes';
RUN;
/* FG3 missing sizes */
PROC FREQ DATA = final_2016;
	TABLES Food_Group_Name_3;
	WHERE SIZE_new in (., 0) ;
	TITLE 'Homescan dataset (2016): Food Group 3 frequency of missing Item sizes';
RUN;

/* 2016 total household weight purchased (kg = g / 1000) = 1,610,918 kg*/
PROC SQL;
TITLE 'Homescan dataset (2016): Total household weight purchased (kg)';
SELECT SUM(QTY*SIZE_new/1000) as WeightPurchased
FROM final_2016;
QUIT;

/* 2016 total household expenditure $9,854,797*/
PROC SQL;
TITLE 'Homescan dataset (2016): Total household expenditure (all items $)';
...
/* $342,883.30 / $9,854,797 total = 3.48% */
PROC SQL;
TITLE 'Homescan dataset (2016): Total household expenditure from items with missing sizes (90,852 items)';
SELECT SUM(QTY*PRICE) as MissingExpenditure,
SUM(QTY*PRICE)/9854797*100 as ExpenditurePercentage
FROM postmissing_2016;
QUIT;
